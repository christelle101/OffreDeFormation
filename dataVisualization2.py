...
# add skill node to the graph
"""
comp_ue = c.competence_ue(p_competence, text)
for item in comp_ue.items():
    if item[0] == "IDU":
        for module in item[1]:
            id_module = get_module_id(module)
            net.add_edge(id_module, 100, weight=1)
    if item[0] == "TC":
        for module in item[1]:
            id_module = get_module_id(module)
            net.add_edge(id_module, 200, weight=1)
